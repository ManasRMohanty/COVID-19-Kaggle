...
bert_model = pickle.load(open("/kaggle/input/chemdner-base-ncbi-bert/chemdner_train_model_complete_base_finetune_all.pkl","rb"))
bert_model.cpu()
...
def process_string_finetune(string_input, padding_length,pubmed_id=""):
    tokenizer = nltk.data.load('tokenizers/punkt/english.pickle')
    sentences = tokenizer.tokenize(string_input)
    
    bert_tokenizer = BertTokenizer.from_pretrained('/kaggle/input/ncbi_bert/')
    ...
    for index in range(len(sentences)):
        ...
        end_index_bert = min(len(sentences),start_index_bert+2*padding_length)
        start_index_bert = max(0,end_index_bert-2*padding_length)
        ...
        for token in pos_tokens:
            ...
            new_dict["pubmed_id"] = pubmed_id
            new_dict["pos_token"] = token[1]
            new_dict["pos_valid"] = True
            
            if(not first_token and re.match(".*[A-Z].*", current_word)):
                new_dict["pos_valid"] = False
